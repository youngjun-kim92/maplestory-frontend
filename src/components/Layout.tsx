...
type NavItem =
  | { type: 'link'; to: string; label: string; icon: string; iconImg?: string; indent?: boolean; hideMobile?: boolean }
  | { type: 'group'; label: string }

const NAV_ITEMS: NavItem[] = [
  { type: 'link',  to: '/dashboard',  label: '대시보드',  icon: '📊' },
  { type: 'group', label: '기록하기' },
  { type: 'link',  to: '/boss',       label: '보스 처치', icon: '⚔️', iconImg: '/maple-icons/boss.png',    indent: true },
  { type: 'link',  to: '/hunting',    label: '사냥',      icon: '🌲', iconImg: '/maple-icons/emblem.png',  indent: true },
    ...
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: 'var(--bg)' }}>
          ...
          {NAV_ITEMS.map((item, idx) => {
            ...
            return (
              ...
              >
                ...
                >
                  {item.iconImg
                    ? <img src={item.iconImg} alt="" width={22} height={22} style={{ imageRendering: 'pixelated', flexShrink: 0 }} />
      ...
      >
        {navLinks.map((item) => (
          ...
          >
            {item.iconImg
              ? <img src={item.iconImg} alt="" width={22} height={22} style={{ imageRendering: 'pixelated' }} />
